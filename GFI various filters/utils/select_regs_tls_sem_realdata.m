...
if sem_model
    % For M=200
%     Lambdas1 = [1e-4 5e-3 0.001 0.05 0.1];
    Lambdas1 = [0.1 0.25 0.5 0.75 1]; %[0.05 0.1 0.15 0.2 0.5];
%     Lambdas2 = [0.075 0.1 0.125 0.15 0.2];
    Lambdas2 = [0.075 0.1 0.125 0.15 0.2];
%     For M=1000
%     Lambdas1 = 1:5;
...
n_signals = 25;
err_H = zeros(length(Lambdas1),length(Lambdas2),n_signals);
err_S = zeros(length(Lambdas1),length(Lambdas2),n_signals);
err_Y = zeros(length(Lambdas1),length(Lambdas2),n_signals);
tic
for i=1:n_signals
    W = triu(rand(N)<eps,1);
    W = W+W';
