...
# %%
def error_metrics(Y_hat, Y_test, H=None, H_gt=None, err_deg=False, err_no_mean=False, return_std=False, print_results=True):
    """
    Calculates the error given the predicted value Y_hat and the real values Y_test
    The type of error returned is given by the parameters:
    * `err_deg`: returns the absolute value of the difference in degrees.
    * `err_no_mean`: removes the mean of each signal before calculating the error in degrees

    If both of these parameters are false, it returns the error normalized by the norm
    of each graph signal

    If H and H_gt are also provided, it also returns the error in the filter.
    """
    if err_no_mean:
        Y_test = Y_test.copy() - Y_test.mean(1)[:,None]
        Y_hat = Y_hat.copy() - Y_hat.mean(1)[:,None]
        err_y = np.mean(np.abs(Y_test - Y_hat))
        std_y = np.std(np.abs(Y_test - Y_hat))
    elif err_deg:
        err_y = np.mean(np.abs(Y_test - Y_hat))
        std_y = np.std(np.abs(Y_test - Y_hat))
    else:
        norm_y = (Y_test**2).sum(0)#np.sqrt((Y_test**2).sum(1))
        err_y_all = ((Y_test - Y_hat)**2).sum(0) / norm_y
        err_y = np.mean(err_y_all)
        std_y = np.std(err_y_all)
...
parser.add_argument("--n_steps",  type=int, default=1, help='Number of steps in the future to use for the prediction')
...
train_test_splits = [1/24, 1/12, 1/8, 1/6, 1/4, 1/3, 1/2, 2/3, 5/6] # 15d, 1m, 1.5m, 2m, 3m, 4m, 6m, 8m, 10m
...
errs_y = np.zeros((n_exps, len(train_test_splits)))
errs_y_deg = np.zeros((n_exps, len(train_test_splits)))
errs_y_0mean = np.zeros((n_exps, len(train_test_splits)))
errs_y_all = np.zeros((n_exps, len(train_test_splits)))
stds_err_all = np.zeros((n_exps, len(train_test_splits)))
errs_H = np.zeros((n_exps, len(train_test_splits)))
...
errs_y_sev = np.zeros((n_exps_sev, len(train_test_splits)))
errs_H_sev = np.zeros((n_exps_sev, len(train_test_splits)))
...
for i, tts in enumerate(train_test_splits):
    print(f"Experiment {i+1}/{len(train_test_splits)}: train test split: {tts} - ", flush=True, end="")
    ...
    errs_y_all[j,i], stds_err_all[j,i] = error_metrics(y_ls_perf_all, Y, return_std=True, print_results=False)
    ...
    Y_hat_ls_all = H_ls @ X
    errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_ls_all, Y, return_std=True, print_results=False)
    ...
    if do_ls_gf:
        ...
        errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_ls_gf_all, Y, return_std=True, print_results=False)
    ...
    if do_eye:
        ...
        Y_eye_all = H_eye @ X
        errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_eye_all, Y, return_std=True, print_results=False)
    ...
    errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_iter_all, Y, return_std=True, print_results=False)
    models.append("RGFI")
    j += 1
    ...
    if do_rew:
        ...
        errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_iter_rew_all, Y, return_std=True, print_results=False)
    ...
    if do_tls:
        ...
        errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_tls_all, Y, return_std=True, print_results=False)
    ...
    if do_scp:
        ...
        errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_scp_all, Y, return_std=True, print_results=False)
        models.append("SCP")
        j += 1
    ...
    if do_var:
        ...
        errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_perf_all, Y_var, return_std=True, print_results=False)
        ...
        errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_var_ls_all, Y_var, return_std=True, print_results=False)
        ...
        if do_ls_gf:
            ...
            errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_var_ls_gf_all, Y_var, return_std=True, print_results=False)
        ...
        errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_var_iter_all, Y_var, return_std=True, print_results=False)
        ...
        if do_rew:
            ...
            errs_y_all[j,i], stds_err_all[j,i] = error_metrics(Y_hat_var_iter_rew_all, Y_var, return_std=True, print_results=False)
    ...
    if do_sev:
        ...
        Hs_iter_rew = []
        for t in range(n_feat):
            _, H, _ = opt.estH_iter(Xs_train[t,:,:], Ys_train[t,:,:], S, Cs[t,:,:], params)
            Hs_iter.append(H)
            if do_rew:
                _, H, _ = opt.estH_iter_rew(Xs_train[t,:,:], Ys_train[t,:,:], S, Cs[t,:,:], params_rew)
...
## Saving results
np.savez(f"{path}/results", errs_y=errs_y, errs_y_deg=errs_y_deg, errs_y_0mean=errs_y_0mean, errs_y_all=errs_y_all, stds_err_all=stds_err_all,
    errs_H=errs_H, models=models, errs_y_sev=errs_y_sev, errs_H_sev=errs_H_sev, models_sev=models_sev,
    norm_data=cli_args.norm_data, P=P, attrs=attrs, gso_type=cli_args.gso_type, stationary=cli_args.stationary,
    monthly_mean=cli_args.monthly_mean, n_steps=n_steps
)
...
for i,m in enumerate(models):
    ax.semilogy(train_test_splits, errs_y[i,:], label=m)
...
for i,m in enumerate(models):
    ax.semilogy(train_test_splits, errs_H[i,:], label=m)
...
if do_sev:
    ...
    for i,m in enumerate(models_sev):
        ax.semilogy(train_test_splits, errs_y_sev[i,:], label=m)
    ...
    for i,m in enumerate(models_sev):
        ax.semilogy(train_test_splits, errs_H_sev[i,:], label=m)
